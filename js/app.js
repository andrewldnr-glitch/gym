...
.opal-ring.activity-rings-host .activity-legend-value{ font-variant-numeric: tabular-nums; opacity:.9; }
  ...
  function mountActivityRings() {
    ...
    // Safety: этот модуль рисует HTML (div-легенду) рядом с SVG-кольцами.
    // Если .opal-ring вёрсткой является <svg> (как на главной), то ничего не делаем,
    // чтобы не ломать существующую SVG-анимацию.
    try {
      const isSvg = (typeof SVGElement !== 'undefined' && container instanceof SVGElement)
        || container.namespaceURI === 'http://www.w3.org/2000/svg';
      if (isSvg) return;
    } catch (_) {}
